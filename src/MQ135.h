...
#define _COEF_A0 110.47
#define _COEF_A1 -2.862
...
need to calibrate device by knowing R0 -- in clean air, approx 10ppm, ratio is
	Rs / R0 = 5.0
*/
#define _CALIBRATION_CONSTANT 3.6

class MQ135 {
public:
	MQ135(int pin) : a_pin(pin), v_in(5.0) { 
		;	// 5.0 Volts assumed / recommended
	}
	MQ135(int pin, float v_in) : a_pin(pin), v_in(v_in) {
		;
	}
